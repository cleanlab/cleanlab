...
def find_label_issues_thread(
    ...
    *,
    ...
    # deleted a bunch of start up code
    ...
    if filter_by in ["prune_by_noise_rate", "prune_by_class", "both"]:
        ...
        global pred_probs_by_class, prune_count_matrix_cols, _min_examples_per_class
        ...
        _min_examples_per_class = min_examples_per_class
        ...
        if n_jobs > 1:
            ...
            with ThreadPoolExecutor(max_workers=n_jobs) as executor:
                ...
                while completed < K:
                    n = len(futures)
                    if n < n_jobs:
                        for k in itertools.islice(jobs, n_jobs-n):
                            futures.append(executor.submit(_prune_by_count_thread, k))
                    done, not_done = wait(futures, return_when=FIRST_COMPLETED)
                    for future in done:
                        label, mask = future.result()
                        label_issues_masks_per_class[label] = mask
                        completed += 1
                        futures = list(not_done)
            for k, mask in label_issues_masks_per_class.items():
                if len(mask) > 1:
                    label_issues_mask[labels==k] = mask
        else:
            label_issues_masks_per_class = [_prune_by_count_thread(k)[1]
                                            for k in range(K)]
            for k, mask in enumerate(label_issues_masks_per_class):
                #print(f"num with label {k} = {np.sum(labels==k)}, labels={labels}")
...
def find_label_issues_cow_spawn(
    labels: LabelLike,
    pred_probs: np.ndarray,
    *,
    return_indices_ranked_by: Optional[str] = None,
    ...
    if filter_by == "prune_by_noise_rate" or filter_by == "both":
        if n_jobs > 1:
            with multiprocessing.Pool(n_jobs) as p:
                label_issues_masks_per_class = p.map(_prune_by_count_cow_spawn,
                                                      args, chunksize= K // n_jobs)
        else:
            label_issues_masks_per_class = [_prune_by_count_cow_spawn(args[k])
                                            for k in range(K)]
...
def find_label_issues_naive(
    ...
    *,
    ...
    global _labels, _label_counts, _prune_count_matrix, _pred_probs, _min_examples_per_class
    _labels = labels
    _label_counts = label_counts
    _prune_count_matrix = prune_count_matrix
        ...
        if n_jobs > 1:  # parallelize
            with multiprocessing.Pool(n_jobs) as p:
                ...
                if big_dataset and tqdm_exists:
                    ...
                    label_issues_masks_per_class = p.map(_prune_by_class_naive, range(K))
        else:  # n_jobs = 1, so no parallelization
            label_issues_masks_per_class = [_prune_by_class_naive(k) for k in range(K)]
        ...
        if n_jobs > 1:  # parallelize
            with multiprocessing.Pool(n_jobs) as p:
                ...
                if big_dataset and tqdm_exists:
                    ...
                    label_issues_masks_per_class = p.map(_prune_by_count_naive, range(K))
        else:  # n_jobs = 1, so no parallelization
            label_issues_masks_per_class = [_prune_by_count_naive(k) for k in range(K)]
...
# that ignores multi labels
# does not use shared memory
# uses fork for multiprocessing to share data
def find_label_issues_cow(
    labels: LabelLike,
    pred_probs: np.ndarray,
    *,
    return_indices_ranked_by: Optional[str] = None,
    ...
    if multi_label:
      ...
      #  return(0,  _find_label_issues_multilabel(
      #      labels,
      #      pred_probs,
    ...
    if filter_by in ["prune_by_noise_rate", "prune_by_class", "both"]:
        ...
        global prune_count_matrix_cols
        prune_count_matrix_cols = {k: prune_count_matrix[:,k] for k in range(K)}
        global _min_examples_per_class
        _min_examples_per_class = min_examples_per_class
        ...
        if n_jobs > 1:  # parallelize
            with multiprocessing.Pool(n_jobs) as p:
                ...
                if big_dataset and tqdm_exists:
                    ...
                    label_issues_masks_per_class = p.map(_prune_by_class_cow, range(K))
        else:  # n_jobs = 1, so no parallelization
            label_issues_masks_per_class = [_prune_by_class_cow(k) for k in range(K)]
...
def _prune_by_class_naive(k:int) -> np.ndarray:
    if _label_counts[k] > _min_examples_per_class:  # No prune if not at least min_examples_per_class
        ...
        # Get return_indices_ranked_by of the smallest prob of class k for examples with noisy label k
        label_filter = _labels == k
        class_probs = _pred_probs[:, k]
        rank = np.partition(class_probs[label_filter], num_issues)[num_issues]
        ...
        return np.zeros(len(_labels), dtype=bool)

def _prune_by_class_cow(k:int) -> np.ndarray:
    pred_probs_k = pred_probs_by_class[k]
    label_counts = pred_probs_k.shape[0]
    if label_counts > _min_examples_per_class:
        num_issues = label_counts - prune_count_matrix_cols[k][k]
        # Get return_indices_ranked_by of the smallest prob of class k for examples with noisy label k
        class_probs = pred_probs_k[:, k]
        rank = np.partition(class_probs, num_issues)[num_issues]
        return (class_probs < rank)
    else:
        warnings.warn(f"class {k} does not have enough examples to prune.")
...
def _prune_by_count_thread(k:int)->Tuple[int, np.ndarray]:
    pred_probs_k = pred_probs_by_class[k]
    label_counts = pred_probs_k.shape[0]
    label_issues_mask = np.zeros(label_counts, dtype=bool)
    #print(f"prune count starting, k={k}")
    if label_counts <= _min_examples_per_class:
        warnings.warn(f"class {k} does not have enough examples to prune.")
        return np.zeros(label_counts, dtype=bool)
    else:
        K = pred_probs_k.shape[1]
        #print(f"pred_probs shape {pred_probs_k.shape}, time={time.time()-start}")
        if K < 1:
        ...
        #cuts_total = 0
        #for j in range(K):
        #    num2prune = prune_count_matrix_cols[k][j]
        ...
        #        margin = pred_probs_k[:, j] - pred_probs_k[:, k]
        #        start = time.time()
        #        cut = -np.partition(-margin, num2prune - 1)[num2prune - 1]
        #        cuts_total += time.time() - start
        #        label_issues_mask = label_issues_mask | (margin >= cut)
        #print(f"cuts took {cuts_total}")
        #return (k, label_issues_mask)

def _prune_by_count_cow_spawn(arg) -> np.ndarray:
    k, min_examples_per_class, arrays = arg
    if arrays is None:
...
def _prune_by_count_naive(k:int) ->np.ndarray:
    ...
    pred_probs_k = _pred_probs[:, k]
    K = get_num_classes(_labels, _pred_probs)
    if _label_counts[k] <= _min_examples_per_class:  # No prune if not at least min_examples_per_class
        warnings.warn(
    ...
    for j in range(K):  # j is true label index (k is noisy label index)
        num2prune = _prune_count_matrix[j][k]
        # Only prune for noise rates, not diagonal entries
        if k != j and num2prune > 0:
            # num2prune's largest p(true class k) - p(noisy class k)
            # for x with true label j
            margin = _pred_probs[:, j] - pred_probs_k
            label_filter = _labels == k
            cut = -np.partition(-margin[label_filter], num2prune - 1)[num2prune - 1]
...
def _prune_by_count_cow(k:int) -> np.ndarray:
    pred_probs_k = pred_probs_by_class[k]
    label_counts = pred_probs_k.shape[0]
    label_issues_mask = np.zeros(label_counts, dtype=bool)
    if label_counts <= _min_examples_per_class:
