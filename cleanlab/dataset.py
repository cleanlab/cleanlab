...
def rank_classes_by_label_quality(
    ...
    **Parameters**: For parameter info, see the docstring of `~cleanlab.dataset.find_overlapping_classes`.
    ...
    given_conditional_noise = given_label_noise / np.clip(joint.sum(axis=1), a_min=EPSILON, a_max=None)
    # p(s!=k, y=k) / p(y=k) , avoiding division by 0:
    true_conditional_noise = true_label_noise / np.clip(joint.sum(axis=0), a_min=EPSILON, a_max=None)
    df = pd.DataFrame(
        {
