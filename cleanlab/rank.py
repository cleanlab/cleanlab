...
def _validate_ensemble_inputs(
    ...
    *,
    weight_ensemble_members_by: str,
    custom_weights: Optional[np.ndarray],
) -> None:
    """Validate inputs for ensemble scoring."""
...
def _compute_log_loss_search_weights(
    labels: np.ndarray,
    pred_probs_list: List[np.ndarray],
    log_loss_search_T_values: List[float],
) -> tuple[np.ndarray, np.ndarray]:
    """Return the best weighted-average pred_probs and corresponding weights."""
    ...
    assert pred_probs_avg_log_loss_weighted is not None
    assert neg_log_loss_weights is not None
    return pred_probs_avg_log_loss_weighted, neg_log_loss_weights


def _compute_ensemble_member_scores_and_accuracy(
    labels: np.ndarray,
    pred_probs_list: List[np.ndarray],
    *,
    method: str,
    ...
    """Compute per-model label quality scores and optional accuracies."""
    ...
    return scores_list, accuracy_list
...
def _aggregate_ensemble_scores(
    ...
    *,
    ...
    custom_weights: Optional[np.ndarray],
    neg_log_loss_weights: Optional[np.ndarray],
    verbose: bool,
) -> np.ndarray:
    """Aggregate per-model scores according to the selected weighting scheme."""

    if weight_ensemble_members_by == "uniform":
        return scores_ensemble.mean(axis=1)  # Uniform weights (simple average)
    ...
    if weight_ensemble_members_by == "accuracy":
        ...
        return (scores_ensemble * weights).sum(axis=1)
    ...
    if weight_ensemble_members_by == "custom":
        ...
        assert len(custom_weights) == scores_ensemble.shape[1], (
            "Length of custom_weights array must match the number of models: len(pred_probs_list)."
        )

        return (scores_ensemble * custom_weights).sum(axis=1)
...
def get_label_quality_ensemble_scores(
    ...
    *,
    ...
    neg_log_loss_weights = None
    if weight_ensemble_members_by == "log_loss_search":
        _, neg_log_loss_weights = _compute_log_loss_search_weights(
            labels, pred_probs_list, log_loss_search_T_values
        )

    scores_list, accuracy_list = _compute_ensemble_member_scores_and_accuracy(
        labels,
        pred_probs_list,
        method=method,
        adjust_pred_probs=adjust_pred_probs,
    ...
    label_quality_scores = _aggregate_ensemble_scores(
        scores_ensemble,
        weight_ensemble_members_by=weight_ensemble_members_by,
        accuracy_list=accuracy_list,
        custom_weights=custom_weights,
        neg_log_loss_weights=neg_log_loss_weights,
        verbose=verbose,
    )
