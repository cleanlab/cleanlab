...
def _downsample_arrays(
    ...
    if h % factor != 0 or w % factor != 0:
        raise ValueError(
            f"Height {h} and width {w} not divisible by downsample value of {factor}. Set kwarg downsample to 1 to avoid downsampling."
        )
...
def _flatten_and_preprocess_masks(
...
def _estimate_ranked_label_issues(
    labels: np.ndarray,
    ...
    *,
    ...
    image_size = np.prod(pred_probs.shape[1:])
    ...
    for start_index in range(0, n, images_per_batch):
        ...
        labels_batch, pred_probs_batch = _flatten_and_preprocess_masks(
            labels[start_index:end_index], pred_probs[start_index:end_index]
        )
...
def _apply_ranked_issues_to_mask(
    ranked_label_issues: np.ndarray,
    labels: np.ndarray,
    pred_probs: np.ndarray,
    h: int,
    w: int,
...
def find_label_issues(
    ...
    **kwargs:
    ...
    # Added Downsampling
    pre_labels, pre_pred_probs = _downsample_arrays(labels, pred_probs, downsample)

    _, _, h, w = pre_pred_probs.shape
    ...
    # )
    ranked_label_issues = _estimate_ranked_label_issues(
        pre_labels, pre_pred_probs, batch_size=batch_size, n_jobs=n_jobs, verbose=verbose
    )

    # Upsample carefully maintaining indicies
    return _apply_ranked_issues_to_mask(
        ranked_label_issues,
        labels,
        pred_probs,
        h,
