...
class SpuriousCorrelations:
    ...
    def spurious_correlations(self) -> pd.DataFrame:
        ...
        image_properties = ['outlier','near_duplicate','non_iid','low_information','dark','blurry','light','odd_aspect_ratio','odd_size']
        property_scores = {}
        for property_of_interest in image_properties:
            if self.issue_summary[self.issue_summary['issue_type'] == property_of_interest]['num_issues'].values[0] > 0:
    ...
    def calculate_spurious_correlation(self, property_of_interest: str, baseline_accuracy: float, cv_folds: int = 5) -> float:
